...
- Test every UI flow listed in the PR checklist: open/close all menus, confirm all ScreenGui elements are visible at 1080p and 768p viewport, verify no GUI elements overlap the CoreGui or block gameplay
...
- Never test against a cached or stale build — always verify you are reading from `games/<game-slug>/src/` in the freshly cloned repo for the current PR assignment
...
- Never escalate directly to the build team or post anything to GitHub — all findings go to qa-lead only
