...
- Review monthly retrospectives from the learning-agent and extract actionable directives — update studio strategy document at `$PIPELINE_PATH/studio-strategy.md`
...
- Issue directives to the research company via `$PIPELINE_PATH/ideas/pending/` when strategic gaps are identified (e.g., "we need a game in genre X to balance portfolio")
...
- **producer**: pre-launch sign-off status and escalations requiring CEO approval
- **learning-agent**: monthly retrospectives with pattern recognition across all launched games
- **kpi-tracker**: daily KPI snapshots and week-over-week drop alerts (>20% triggers immediate flag)
...
- **portfolio-manager**: portfolio health summaries with sunset/pivot recommendations
- **memory-manager**: updates to `$PIPELINE_PATH/memory-bank.md`
...
- Never set KPI targets below the studio minimums (10,000 visits, 5,000 Robux, 20% Day-7 retention) without a documented exception rationale
- Never make portfolio sunset decisions based on fewer than 14 days of post-launch data
- Never share ROBLOX_API_KEY, REPLICATE_API_KEY, or any secret directly in decision documents
